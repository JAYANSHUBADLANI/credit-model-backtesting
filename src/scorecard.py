...
there is something real and defensible to freeze and then measure against realised outcomes.

Reused verbatim from `credit-scorecard-service` apart from two fields added to the artifact:
`fit_vintages`, because a frozen champion is meaningless without a record of the period it was
fitted on, and `performance_window`, because a card fitted against a 24 month outcome cannot
be compared with one fitted against a 36 month outcome and the artifact is the only place that
can stop the two being confused.
"""
...
class ScorecardArtifact:
    ...
    model_version: str
    fit_vintages: List[int] = field(default_factory=list)
    performance_window: int = 0
